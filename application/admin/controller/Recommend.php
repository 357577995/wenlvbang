...
class recommend extends Base
{
	...
	public function cate ()
	{
		...
		return $this->fetch('merecommendate', [
	...
	public function cate_add ()
	{
		// 检查权限
		// admin_role('meeting_cate');
		return $this->fetch('recommend/cate_add');
	}
	...
	public function cate_insert ()
	{
		...
		if(! empty($cate))
		{
			$this->error('该分类名称已存在', '/admin/recommend/cate');
		}
		else
		{
			...
			if($result)
			{
				...
				$this->success('添加成功', '/admin/recommend/cate');
			}
			else
			{
				$this->error('添加失败', '/admin/recommend/cate');
			}
		}
	}
	...
	public function cate_del ()
	{
		...
		if($res)
		{
			...
			$this->success('删除成功', '/admin/recommend/cate');
		}
		else
		{
			$this->error('删除失败', '/admin/recommend/cate');
		}
	}
	...
	public function cate_edit ()
	{
		...
		return $this->fetch('recommend/cate_edit', [
			'list' => $list
		]);
	}
	...
	public function cate_update ()
	{
		...
		if($result)
		{
			...
			$this->success('修改成功', '/admin/recommend/cate');
		}
		else
		{
			$this->error('修改失败', '/admin/recommend/cate');
		}
	}
	...
	public function index (Request $request)
	{
		...
		$list = Db::table('recommend')->where($where)->paginate(5, false, [
			'query' => request()->param()
		]);
		$page = $list->render();
		$recommend = [];
		...
		{
			$recommend[] = $vo;
			$recommend[$k]['time'] = date('Y-m-d H:i:s', $vo['time']);
		}
		// 搜索用的分类
		$catelist = Db::table('meeting_cate')->select();
		// 总个数
		$total = Db::table('recommend')->where($where)->field('COUNT(*) as total')->select();
		$this->assign('total', $total[0]['total']);
	...
	public function insert ()
	{
		...
		if($size == '0')
		{
			...
			$data['image'] = $img;
			$result = Db::name('recommend')->insert($data);
			if($result)
			{
				// 添加管理员日志
				admin_log($content = '添加外部推荐活动-' . $_POST['title']);
				$this->success('添加成功', '/admin/recommend/index');
			}
			else
			{
				$this->error('添加失败', '/admin/recommend/index');
			}
		}
		else
		{
			...
			$path = $_SERVER['DOCUMENT_ROOT'] . '/image/admin/recommend/';
			$res = move_uploaded_file($file, $path . $name) ? 'ok' : 'false';
			if($res == 'ok')
			{
				$img = '/admin/recommend/' . $name;
				$data['image'] = $img;
				$result = Db::name('recommend')->insert($data);
				if($result)
				{
			...
			{
				$this->error('文件上传失败', '/admin/recommend/add');
			}
		}
	}
	...
	public function del ()
	{
		...
		// admin_role('new_del');
		$id = $_GET['id'];
		$new = Db::table('recommend')->field('image,title')->where('id', $id)->find();
		...
		if($res)
		{
			...
			$this->success('删除成功', '/admin/recommend/index');
		}
		else
		{
			$this->error('删除失败', '/admin/recommend/index');
		}
	}
	...
	public function edit ()
	{
		...
		// admin_role('new_edit');
		$id = $_GET['id'];
		$new = Db::table('recommend')->where('id', $id)->find();
		// 类别
		$cate = Db::table('meeting_cate')->select();
	...
	public function update ()
	{
		...
		$data['url'] = $_POST['url'];
		$data['time'] = ! empty($_POST['time']) ? strtotime($_POST['time']) : time();
		$oldimg = Db::table('recommend')->field('image')->where('id', $_POST['id'])->find();
		// 判断是否有新图片上传
		$size = $_FILES['image']['size'];
		if($size == '0')
		{
			$result = Db::table('recommend')->where('id', $_POST['id'])->update($data);
			if($result)
			{
				// 添加管理员日志
				admin_log($content = '修改外部推荐活动-' . $_POST['title']);
				$this->success('修改成功', '/admin/recommend/edit?id=' . $_POST['id']);
			}
			else
			{
				$this->error('修改失败', '/admin/recommend/edit?id=' . $_POST['id']);
			}
		}
		else
		{
			...
			if(! in_array($type, $filetype))
			{
				$this->error('图片类型上传不正确，允许上传的图片类型：png,jpg,jpeg,gif', '/admin/recommend/edit?id=' . $_POST['id']);
			}
			$name = time() . rand(1, 9999) . "." . $type;
			$path = $_SERVER['DOCUMENT_ROOT'] . '/image/admin/recommend/';
			$res = move_uploaded_file($file, $path . $name) ? 'ok' : 'false';
			...
			{
				$this->error('文件上传失败', '/admin/recommend/edit?id=' . $_POST['id']);
			}
		}
	}
}

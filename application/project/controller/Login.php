...
class Login extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if($request->post()){
            ...
            if(!captcha_check($code)) {
                ...
                // 获取用户数据信息
                $admin = Db::table('users')->field('id,username,status')->where($data)->where('type',1)->find();
                if (!empty($admin)) {
                    //判断审核是否通过
                    if($admin['status'] != '2'){
                        ...
                        // 设置admin_id
                        Session::set('project_id', $admin['id']);
                        Session::set('project_username', $admin['username']);
                        $this->success('登录成功', 'project/index/index');
                    }
                } else {
                    $this->error('用户名或密码错误');
                }
    ...
    public function logout()
    {
        ...
        Session::delete('project_username');
        $this->success('退出成功', 'project/login/index');
    }
    ...
    public function register_do(Request $request){
        ...
        if($size == '0'){
            $this->error('图片未上传','/project/login/register');
        }else{
            ...
            {  
                $this->error('图片类型上传不正确，允许上传的图片类型：png,jpg,jpeg,gif','/project/login/register');
            }
            $name = time().rand(1,9999).".".$type;   
            $path=$_SERVER['DOCUMENT_ROOT'].'/image/project/users/'; 
            $res = move_uploaded_file($file ,$path.$name)? 'ok' : 'false';  
            if($res == 'ok'){
                $img = '/project/users/'.$name;
                $data['licence'] = $img;
                $result = Db::name('users')->insert($data);
                if($result){
                    $this->success('注册成功，已提交总管理后台进行审核，请等待审核通过后登录。','/project/login/index');
                }else{
                    $this->error('注册失败','/project/login/register');
                }
            }else{
                $this->error('文件上传失败','/project/login/register');
            }
        }
    }
    /**
    ...
    */
    public function isemail(Request $request){
        ...
        $email = trim($email,' ');
        $res = Db::table('users')->where('email',$email)->where('type','1')->find();
        if(!empty($res)){
            //存在
            echo 1;
        }else{
    ...
    */
    public function sendmail(Request $request){
        ...
        $info = Db::table('users')->where('email',$toemail)->where('type','1')->find();
        $token = md5($info['id'].$info['username']);
        $href = "'http://".$_SERVER['SERVER_NAME'].'/project/login/newpassword?id='.$info['id'].'&token='.$token."'";
        $name='文旅帮';
        $subject='项目方-找回密码';
        ...
        if($res){
            $this->success('发送邮件成功','/project/login/index');
        }else{
            $this->error('发送邮件失败');
        }
    }
    ...
    */
    ...
    public function newpassword_do(Request $request){
        ...
        if($res){
            $this->success('修改密码成功','/project/login/index');
        }else{
            $this->error('修改密码失败','/project/login/index');
        }
    }

...
class Order extends Base
{
    ...
    */
    ...
    {
        //判断该用户是否是项目方
        $user = Db::table('users')->field('type,status')->where('id',$_SESSION['think']['user_id'])->find();
        if(!empty($user) && $user['type'] == '1'){
            if($user['status']!='2'){
                $this->error('对不起，你的项目方身份还未审核通过');
            }else{
                //修改之前-把方案库所有数据恢复初始状态
                Db::table('cart')->where('user_id',$_SESSION['think']['user_id'])->where('status',2)->update(['status'=>1]);

                foreach($_POST['cart_id'] as $k=>$vo){
                    $data['status'] = 2;
                    Db::table('cart')->where('id',$vo)->update($data);
                }
                $this->redirect('/index/order/create');
            }
    ...
    */
    public function create(){
        $cart = Db::table('cart')->where('user_id',$_SESSION['think']['user_id'])->where('status',2)->select();
    ...
    */
    public function confirm(Request $request){
        ...
        $data['project_user_id'] = $_SESSION['think']['user_id'];
        $data['status'] = 1;
        $data['addtime'] = time();
        //生成订单
        ...
        if($res){
            ...
            //清空购物车
            Db::table('cart')->where('user_id',$_SESSION['think']['user_id'])->where('status',2)->delete();

            $this->success('创建订单成功','/index/order/index');
        }else{
            $this->error('创建订单失败');
        }
    }
